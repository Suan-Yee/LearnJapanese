...
export type TabType = "kana" | "vocabulary" | "kanji" | "grammar" | "search" | "saved";
const LAST_VOCAB_PATH_KEY = "last_vocab_path";
const LAST_GRAMMAR_PATH_KEY = "last_grammar_path";

export function Navbar() {
  const pathname = usePathname();
  const router = useRouter();
  const lastVocabularyPath = useRef("/vocabulary");
  const lastGrammarPath = useRef("/grammar");
  ...
  useEffect(() => {
    if (!pathname.startsWith("/grammar")) return;

    setCachedPath(LAST_GRAMMAR_PATH_KEY, pathname);
    lastGrammarPath.current = pathname;
  }, [pathname]);
  ...
  const getLastGrammarPath = () => {
    const stored = getCachedPath(LAST_GRAMMAR_PATH_KEY);
    if (stored?.startsWith("/grammar")) return stored;
    return lastGrammarPath.current;
  };
  ...
  const handleGrammarClick = (event: MouseEvent<HTMLAnchorElement>) => {
    if (pathname.startsWith("/grammar")) return;

    event.preventDefault();
    router.push(getLastGrammarPath());
  };
  ...
  const tabs: { id: TabType; label: string; href: string }[] = [
    ...
    { id: "grammar", label: "Grammar", href: pathname.startsWith("/grammar") ? pathname : "/grammar" },
  ...
  return (
    <nav className="sticky top-0 z-50 w-full border-b border-border/50 bg-background/70 backdrop-blur-xl shadow-sm transition-colors relative">
      <div className="mx-auto flex h-16 w-full max-w-5xl items-center justify-between px-4 sm:px-6">
        ...
        {/* Desktop Navigation */}
        <div className="hidden h-full sm:flex sm:min-w-0 sm:flex-1 sm:items-center sm:justify-end sm:gap-2 md:gap-3">
          {tabs.map((tab) => {
            ...
            return (
              <Link
                ...
                onClick={(e) => {
                  ...
                  if (tab.id === "grammar") handleGrammarClick(e);
